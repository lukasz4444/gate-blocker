...
if opts.d then
  ...
  └───────────────────────────┘]])
    BranchURL = "https://raw.githubusercontent.com/MR-Spagetty/gate-blocking-system/dev"
  else
    BranchURL = "https://raw.githubusercontent.com/MR-Spagetty/gate-blocking-system/release"
  end
  ReleaseVersionsFile = "/gbs/releaseVersions.ff"
  ReleaseVersions = nil

function createInstallDirectory() -- Creates `/gbs` directory, if it doesn't already exist
    if not filesystem.isDirectory("/gbs") then
        print("Creating \"/gbs\" directory")
        local success, msg = filesystem.makeDirectory("/gbs")
        if success == nil then
        io.stderr:write("Failed to created \"/gbs\" directory, "..msg)
        os.exit(false)
        end
    end
end
...
function checkForExistingInstall() -- Checks for existing install, and prompt user if one is found
    if filesystem.exists("/gbs/gateBlockingSystem.lua") then
        print([[
    ┌────────────────────────────────────────┐
    │An existing installation of GBS         │
...
function downloadNeededFiles()
    ...
    file = io.open("/gbs/installedVersions.ff", "w")
...
shell.execute("/gbs/gateBlockingSystem.lua "..options)
else
io.stderr:write("GBS is Not Correctly Installed\n")
end
    ]]
    local file = io.open("/bin/gbs.lua", "w")
    file:write(gbsBinFile)
    file:close()
end
...
Please use the 'gbs' system command to run the launcher.
]])